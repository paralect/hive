...
export default (server) => {
    ...
    const getUserData = async (socket) => {
        const accessToken = getCookie(socket.handshake.headers.cookie, "ship_access_token");
        if (!accessToken) {
            logger.info("Note: socket io anonymous auth. Add user authentication in socketIoService");
        }
        return {
            _id: "anonymous",
        };
