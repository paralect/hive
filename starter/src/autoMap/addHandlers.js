...
const getDependentFields = (schema, dependentFieldName) => {
    ...
    return Object.keys(targetSchema.keys).filter((key) => !_.includes(["_id", "createdOn", "updatedOn"], key));
};
...
const getToUpdate = async ({ doc, schemaName }) => {
    ...
    const toUpdate = {};
    await Promise.all(dependentFieldNames.map(async (dependentFieldName) => {
        const { schema: dependentSchemaName } = updatedSchemaMappings[schemaName][dependentFieldName];
        const { dependentFields } = updatedSchemaMappings[schemaName][dependentFieldName];
        if (!_.isEmpty(doc[dependentFieldName])) {
            if (_.isArray(doc[dependentFieldName])) {
                toUpdate[dependentFieldName] = (await db.services[dependentSchemaName].find({
                ...
                toUpdate[dependentFieldName] = await db.services[dependentSchemaName].findOne({
...
const addOnDependentEntitiesUpdatedHandlers = ({ schemaName }) => {
    const dependentFieldNames = Object.keys(schemaMappings[schemaName]);
    dependentFieldNames.forEach((dependentFieldName) => {
        const dependentFieldSchemaName = schemaMappings[schemaName][dependentFieldName].schema;
        const schema = db.schemas[schemaName].describe().keys;
        const dependentFields = getDependentFields(schema, dependentFieldName);
        db.services[dependentFieldSchemaName].on("updated", ifUpdated(dependentFields, async ({ doc }) => {
            const toUpdate = _.pick(doc, ["_id", ...dependentFields]);
            if (schema[dependentFieldName].type === "array") {
                db.services[schemaName].atomic.update({ [`${dependentFieldName}._id`]: doc._id }, {
                    $set: {
