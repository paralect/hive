    ...
    if (!prevSchemaMappings) {
        ...
        await Promise.all(schemaNames.map(async (schemaName) => {
            ...
            await Promise.all(fieldNames.map(async (fieldName) => {
                ...
                if (_.difference(dependentFields, prevDependentFields).length !== 0) {
                    console.log(`Mapping schema changes: ${schemaName}.${fieldName}`);
                    const uniqueDependentEntityIds = await db.services[schemaName].distinct(`${fieldName}._id`);
                    const { results: uniqueDependentEntities } = await db.services[schemaMappings[schemaName][fieldName].schema].find({
                        _id: { $in: uniqueDependentEntityIds },
                    }, { fields: ["_id", ...dependentFields] });
                    await Promise.all(uniqueDependentEntities.map(async (entity) => {
                        if (schema[fieldName].type === "array") {
                            await db.services[schemaName].atomic.update({ [`${fieldName}._id`]: entity._id }, {
        ...
        schemaMappingService.atomic.update({ _id: prevSchema._id }, { $set: { mappings: joiSchemaToSchemaMappings() } });
    }
};
