...
import MongoServiceError from "./mongo-service-error.js";
...
class MongoService extends MongoQueryService {
    ...
    static _deepCompare(data, initialData, properties) {
        ...
        if (Array.isArray(properties)) {
            ...
            Object.keys(properties).forEach((prop) => {
                ...
                const value = _.get(data, prop);
                const initialValue = _.get(initialData, prop);
                if (_.isEqual(value, properties[prop]) &&
    ...
    async _validate(entity) {
        if (this._options.validate) {
            ...
            if (error) {
                ...
                throw new MongoServiceError(MongoServiceError.INVALID_SCHEMA, `Document schema is invalid: ${JSON.stringify(error, null, 2)}`, error);
            }
            return value;
        }
        return entity;
    ...
    async create(objs, options = {}) {
        ...
        let created = await Promise.all(entities.map(async (doc) => {
    ...
    async updateOne(query, updateFn, options = {}) {
        if (!_.isFunction(updateFn)) {
            throw new MongoServiceError(MongoServiceError.INVALID_ARGUMENT, `updateOne: second argument is invalid. Expected a function but got ${typeof updateFn}`);
        }
        const findOptions = {};
        if (options.session)
        ...
        if (!doc) {
            throw new MongoServiceError(MongoServiceError.NOT_FOUND, `updateOne: document not found. Query: ${JSON.stringify(query)}`);
        }
    ...
    async updateMany(query, updateFn, options = {}) {
        if (!_.isFunction(updateFn)) {
            throw new MongoServiceError(MongoServiceError.INVALID_ARGUMENT, `updateMany: second argument is invalid. Expected a function but got ${typeof updateFn}`);
        }
        const findOptions = {};
        if (options.session)
            findOptions.session = options.session;
        ...
        await Promise.all(updated.map((doc) => this._collection.update({ ...query, _id: doc._id }, { $set: doc }, options)));
    ...
    async performTransaction(transactionFn, options = {}) {
        if (!_.isFunction(transactionFn)) {
            throw new MongoServiceError(MongoServiceError.INVALID_ARGUMENT, `performTransaction: first argument is invalid. Expected a function but got ${typeof transactionFn}`);
        }
